    ...
    return (
        <footer className="mt-8">
            ...
            <div className="grid grid-cols-2 gap-4 mt-8 mb-16">
                {prevPost ? (
                    <Link
                        href={`/blog/${prevPost.categoryPublicName}/${prevPost.slug}`}
                        className="col-start-1 no-underline"
                    >
                        <Card className="h-full transition-all duration-300 ease-in-out shadow-none hover:shadow-md hover:-translate-y-1">
                            <div className="flex items-center h-full overflow-hidden">
                                ...
                                <div className="flex-grow min-w-0">
                                    <CardHeader className="text-sm text-gray-500 dark:text-gray-400 pb-2 pl-1">
                                        이전 포스트
                                    </CardHeader>
                    ...
                    <div className="col-start-1 flex items-center h-full text-gray-500 dark:text-gray-400">
                        이전 글이 없습니다
                    </div>
                )}
                ...
                {nextPost ? (
                    <Link
                        href={`/blog/${nextPost.categoryPublicName}/${nextPost.slug}`}
                        className="col-start-2 no-underline"
                    >
                        <Card className="h-full transition-all duration-300 ease-in-out shadow-none hover:shadow-md hover:-translate-y-1">
                            <div className="flex items-center h-full overflow-hidden">
                                <div className="flex-grow min-w-0">
                                    <CardHeader className="text-sm text-gray-500 dark:text-gray-400 pb-2 text-right pr-1">
                                        다음 포스트
                                    </CardHeader>
                    ...
                    <div className="col-start-2 flex items-center justify-end h-full text-gray-500 dark:text-gray-400">
                        다음 글이 없습니다
                    </div>
                )}
            </div>
